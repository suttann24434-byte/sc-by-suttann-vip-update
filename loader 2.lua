...
--// Fitur: Intro GUI + Sound Effect + Key System + Auto Save + 1 Key per Device
...
Ambient.SoundId = "rbxassetid://5410086212" -- ambient sci-fi
Ambient.Volume = 0.6
Ambient.Looped = true
Ambient:Play()

local Whoosh = Instance.new("Sound", IntroFrame)
Whoosh.SoundId = "rbxassetid://9127401354" -- whoosh effect
Whoosh.Volume = 1
...
-- Efek teks bergelombang
task.spawn(function()
    while IntroText.Parent do
        for i = 1, #IntroText.Text do
            local start = string.sub(IntroText.Text, 1, i-1)
            local mid = string.sub(IntroText.Text, i, i)
            local last = string.sub(IntroText.Text, i+1)
            IntroText.Text = start .. "<b><font color='rgb(0,200,255)'>" .. mid .. "</font></b>" .. last
